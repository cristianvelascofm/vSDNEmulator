...
// Lienzo canvas
var canvas = (void 0)._canvas = new fabric.Canvas('canvas');
canvas.backgroundColor = '#EDFAFA';
canvas.renderAll();
var insertOp = false;
var imgUrl = ''; // Declaración de Variables
...
var topologyType = ""; //Variables para la ejecucion de las Topologías desde Templates

var numHostTemplate = 0;
var numDepthTemplate = 0;
var numFanoutTemplate = 0; // Zoom Lienzo Mediante el Moviemiento del Scroll del Mouse
...
function insertElement(x, y, image, tag, numHost, topology) {
  ...
  elemento.set({
    ...
    id: tag,
    elementConnection: [],
    // Contiene las conexiones pertenecientes al elmento
  ...
  if (topology == "single") {
    if (tag.charAt(0) == 'h') {
      ...
      for (var i = 0; i < numHost; i++) {
        ...
        if (asociate.charAt(0) == "s") {
          ...
          var li = elemento.connectionLine[i]; // Asignación de lineas por cada puerto en el grupo
      ...
      canvas.add(groupController); //groupController.
    ...
    if (tag.charAt(0) == 'h') {
      ...
      for (var i = 0; i < 2; i++) {
        ...
        groupHostPort.connection.push(groupHost.connection[i]);
        var l = groupHost.connection[i];
      ...
      for (var i = 0; i < 2; i++) {
        ...
        var line = groupSwitch.connection[i];
        groupSwitchPort.connection.push(groupSwitch.connection[i]);
        groupSwitchPort.line = line;
        canvas.add(groupSwitch.connection[i]);
        canvas.add(groupSwitchPort);
      }
      ...
      for (var i = 0; i < numHost; i++) {
        connection.type = "association";
        connection.elementOrigin = tag;
        connection.x2 = numHost * 265 / 2 - 50;
        connection.y2 = y + 35;
        connection.x1 = i * 183 + 346;
        connection.y1 = y + 155;
        connection.id = "a" + i;
        var link = makeLink([connection.x1, connection.y1, connection.x2, connection.y2], "normal");
        groupController.connection.push(link);
...
function linkMaker(topologyType) {
  ...
  if (topologyType == "linear") {
    ...
    for (var l = 0; l < netInfo.length; l++) {
      ...
      console.log("Conexiones:" + k); // puntos iniciales del switch
...
function topologyMaker(numHost, topologyType, depth, fanout) {
  var posX = [];
  var image = ""; //var pSX = (tagHost.length * 68);
  ...
  var pCY = 20;
  var h = [];
  var s = []; // Creador Topología Single - 1 Conmutador Conectado a N Host
  ...
  if (topologyType == "single") {
    if (numHost < 2) {
      ...
      netInfo.push(objt);
      linkMaker(topologyType); // Agrega el Link del Controller
      ...
      insertElement(101, pSY, '../static/img/openflow_switch.png', "s" + (tagSwitchOF.length + 1), numHost, topologyType); //Inserta el Controller de la Red Single
      ...
      if (numHost < 2) {
        ...
        var pCX1 = netInfo.length * 68; // Inserta Switch y Host de la Topolgía Linear
      ...
      for (var i = tagHost.length - numHost; i <= tagHost.length; i++) {
        ...
        canvas.forEachObject(function (obj) {
          if (obj.elementContainer && obj.elementContainer == id0[i - 1]) {
            if (obj.id) posX2.push(obj.connection[0].x2);
            posY2.push(obj.connection[0].y2);
            objHost.push(obj);
          }
        });
...
function insertElementClick(x, y, image, tag) {
  ...
  if (tag.charAt(0) == 'h') {
    ...
    var port = new Image();
    port.src = "../static/img/port.png"; // Creación de puertos por cada enlace.
...
var btnCursor = document.getElementById('cursor'); //btnCursor.focus;
