...
//Almacena el protocolo de trafico q se ejecuta 
var protocolTrafficActual = '';


var datosYNumBytesServer = [];
var datosYBitsPerSecondServer = [];
...
$('#generateBtn').on('click', function () {
    ...
    if (typeTraffic == 'TCP') {
        trafficDir['TCP'] = 'true';
        protocolTrafficActual = 'TCP';
    } else {
        trafficDir['UDP'] = 'true';
        protocolTrafficActual = 'UDP';
    }
            ...
            if (protocolTrafficActual == 'TCP') {
                for (var k in data) {
                    ...
                    if (String(sp[1]).charAt(0) == 'h') {
                        ...
                        //Promedio de Bytes Transmitidos en la Emulacion del Trafico Cliente
                        ...
                        for (var t in data[k]['speciffic']) {
                            ...
                            totalBytesTxServer = totalBytesTxServer + parseInt(data[k]['speciffic'][t]['n_bytes']);
                            bitsPerSecondServer = bitsPerSecondServer + parseInt(data[k]['speciffic'][t]['bits_per_second']);
                            ...
                            //Valores Numero de Bytes
                            if (Object.keys(trafficValuesServer).includes(String(t) + '_num_bytes')) {

                                var valActualNumBytes = parseInt(trafficValuesServer[llave + '_num_bytes']);
                                var valorSumar = parseInt(data[k]['speciffic'][t]['n_bytes']);
                                ...
                                trafficValuesServer[llave + '_num_bytes'] = parseInt(data[k]['speciffic'][t]['n_bytes']);

                            }


                            //Valores Bits por Segundo
                            if (Object.keys(trafficValuesServer).includes(String(t) + '_bits_per_second')) {

                                var valActualNumBytes = parseInt(trafficValuesServer[llave + '_bits_per_second']);
                                var valorSumar = parseInt(data[k]['speciffic'][t]['bits_per_second']);

                                var totalNumBytes = valActualNumBytes + valorSumar;
                                trafficValuesServer[llave + '_bits_per_second'] = totalNumBytes;

                            } else {
                                trafficValuesServer[llave + '_bits_per_second'] = parseInt(data[k]['speciffic'][t]['bits_per_second']);
...
$('.graphic').on('click', function () {
    ...
    datosYNumBytesServer.push(0);
    datosYBitsPerSecondServer.push(0);
...
/* Selecctor clien o server  */
$('#inputSelectorSeverClient').on('click', function () {

    var seleccion = $('#optionSelectServerClientTCP option:selected').text();
    ...
    if (seleccion == "Servidor") {
        ...
        var graphicsServer = new Chart(graphServer, {
            type: 'line',
            data: {

                labels: labelsGraphic,
                datasets: [{
                    label: 'Total de Bytes Recibidos',
                    data: datosYNumBytesServer,
                    backgroundColor: [
                ...
                },
                {
                    ...
                    data: datosYBitsPerSecondServer,
                    backgroundColor: [
                        'rgba(111, 194, 63, 0.2)'
                    ],
                    borderWidth: 1,
        ...
        if (protocolTrafficActual == 'TCP') {
            ...
            $('#containerFancyInfoGraphicTCPServer').show();
            ...
            $('#containerFancyInfoGraphicUDP').hide();
            $('#containerFancyInfoGraphicUDPServer').show();
        }
...
function fancyInfoTextArea(facnyOrigin) {
    ...
    } else if (facnyOrigin == 'port') {

        titleInfo.text('PUERTO');
        tableAreaInfo.append('<tr><td>' + 'Dirección IP' + '</td> <td>' + 'Conjunto de números que identifica, de manera lógica y jerárquica, a una interfaz en la red de un dispositivo que utilice el protocolo o, que corresponde al nivel de red del modelo TCP/IP.' + '</td></tr>');
    } else if (facnyOrigin == 'label') {
        titleInfo.text('ETIQUETA');
        tableAreaInfo.append('<tr><td>' + 'Etiqueta' + '</td> <td>' + ' Rótulo o nombre que presenta información considerada relevante para un determinado elemento de red.' + '</td></tr>');
    } else if (facnyOrigin == 'controller') {

        titleInfo.text('controller');
        tableAreaInfo.append('<tr><td>' + 'Tipo' + '</td> <td>' + 'Permite seleccionar la entidad lógica de control (Controlador SDN) encargada de traducir las peticiones de las aplicaciones a las rutas de datos, dando a la capa de aplicación una visión abstracta de la red mediante estadísticas y posibles eventos.' +
            '<li>OpenFlow Reference Implementation. Controlador software OpenFlow lógicamente centralizado que permite modificar el comportamiento de los dispositivos de red a través de un conjunto de instrucciones de reenvío definidas.</li>' +
            '<li>NOX: Plataforma de control de red, que proporciona una interfaz programática de alto nivel para la gestión y el desarrollo de aplicaciones de control de red.</li>' +
            '<li>OVS Controller: Open vSwitch Controller. Software de código abierto, diseñado para ser utilizado como un switch virtual en entornos de servidores virtualizados. Es el encargado de reenviar el tráfico entre diferentes máquinas virtuales (VMs) en el mismo host físico y también reenviar el tráfico entre las máquinas virtuales y la red física.</li>' +
            '<li>ODL: OpenDayLight  Controller. Proyecto colaborativo de código abierto, que implementa una plataforma para la creación de redes definidas por software para la supervisión de dispositivos de red abierta y centralizada.</li>' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Dirección IP' + '</td> <td>' + 'Conjunto de números que identifica, de manera lógica y jerárquica, a una interfaz en la red de un dispositivo que utilice el protocolo o, que corresponde al nivel de red del modelo TCP/IP.' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Puerto' + '</td> <td>' + 'Interfaz virtual a través de la cual se pueden enviar y recibir los diferentes tipos de datos.' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Protocolo' + '</td> <td>' + 'Sistema de reglas que permiten que dos o más entidades de un sistema de comunicación se comuniquen entre ellas para transmitir información.' +
            '<li>TCP: Transmission Control Protocol.  Protocolo de control de transmisión orientado a conexión, es decir, los datos pueden enviarse de forma bidireccional una vez establecida la conexión.</li>' +
            '<li>SSL: Secure Sockets Layer. Protocolo para navegadores y servidores web que permite autenticar, cifrar y descifrar la información enviada a través de Internet..</li>' + '</td></tr>');
    } else if (facnyOrigin == 'switch') {

        titleInfo.text('switch');
        tableAreaInfo.append('<tr><td>' + 'Tipo' + '</td> <td>' + 'Permite seleccionar el tipo de dispositivo virtual encargado de la conexión de maquinas virtules y periféricos a la red con el fin de comunicarse entre sí y con otras redes.' +
            '<li>IVS: Indigo Virtual Switch. Conmutador virtual OpenFlow puro diseñado para un alto rendimiento y una administración mínima. Está construido sobre la plataforma Indigo, que proporciona un núcleo común para muchos conmutadores virtuales.</li>' +
            '<li>Linux Brigde: Puente de Linux que se comporta como un conmutador de red, reenviando paquetes entre interfaces de red virtuales que están conectadas a él. Por lo general, se usa para reenviar paquetes en enrutadores virtuales, puertas de enlace o entre máquinas virtuales y espacios de nombres de red en un host.</li>' +
            '<li>OVS Bridge: Open vSwitch bridge. Puente diseñado para  comunicar un switch virtual en entornos de servidores virtualizados.</li>' +
            '<li>OVS: Open vSwitch. Software de código abierto, diseñado para ser utilizado como un switch virtual en entornos de servidores virtualizados. Es el encargado de reenviar el tráfico entre diferentes máquinas virtuales (VMs).</li>' +
            '<li>User Switch. .</li>' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'STP: Spanning Tree Protocol' + '</td> <td>' + 'Protocolo de red de capa 2 del modelo OSI, encargado de gestionar la presencia de bucles en topologías de red debido a la existencia de enlaces redundantes.' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'STP Priority' + '</td> <td>' + 'Es un valor configurable encarado de establecer la prioridad del switch en un valor más pequeño que el del valor por defecto (32768), el nuevo valor debe ser múltiplo de 4096. Esto sólo se debe implementar cuando se tiene un conocimiento profundo del flujo de tráfico en la red.' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Dirección IP' + '</td> <td>' + 'Conjunto de números que identifica, de manera lógica y jerárquica, a una interfaz en la red de un dispositivo que utilice el protocolo o, que corresponde al nivel de red del modelo TCP/IP.' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'DPCTL Port' + '</td> <td>' + 'Puerto para la administración y control sobre el conmutador OpenFlow, mediante la agregación de flujos a la tabla de flujo, la consulta de las características y el estado del interruptor y el cambio de otras configuraciones. Por defecto es el 6633.' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Protocolo' + '</td> <td>' + 'Sistema de reglas que permite a los controladores de red determinar la ruta de los paquetes de red a través de una red de conmutadores. El protocolo OpenFlow se establecio como la primera interfaz de comunicaciones estándar definida entre las capas de control y reenvío de una arquitectura SDN.' +
            '<li>Versión Openflow 1.1.</li>' +
            '<li>Versión Openflow 1.2.</li>' +
            '<li>Versión Openflow 1.3.</li>' +
            '<li>Versión Openflow 1.4.</li>' +
            '<li>Versión Openflow 1.5.</li>' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Data Path' + '</td> <td>' + ' Ruta lógica para procesar los datos de entrada y generar los datos de salida correctos, generalmente se divide en bloques con el fin de ofrecer la optimización de la velocidad en el procesamiento de datos.' +
            '<li>Kernel: Elemento principal de los sistemas Linux e interfaz fundamental entre el dispsitivo y sus procesos. Comunicandolos entre sí y gestiona los recursos de la manera más eficiente posible.</li>' +
            '<li>User.</li>' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Data Path ID' + '</td> <td>' + 'Identificador de ruta de datos, determinado por un número de 64 bits conformado por los 48 bits inferiores destinados a la dirección MAC del conmutador y los 16 bits superiores destinados al implementador.' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Data Path Args' + '</td> <td>' + ' ' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Modelo de Fallas' + '</td> <td>' + 'Determina una causa de falla o una posible manera en la que un sistema puede fallar.' +
            '<li>Ninguno</li>' +
            '<li>Secure</li>' +
            '<li>Standalone</li>' + '</td></tr>');

    } else if (facnyOrigin == 'IPgenerator') {

        titleInfo.text('IP Servidor Grafico');
        tableAreaInfo.append('<tr><td>' + 'Dirección IP' + '</td> <td>' + 'Conjunto de números que identifica de manera lógica y jerárquica la interfaz de red del dispositivo local que utiliza como servidor Grafico.' + '</td></tr>');
    } else if (facnyOrigin == 'trafficInfo') {

        titleInfo.text('Analizador de Tráfico Especifico');
        tableAreaInfo.append('<tr><td>' + 'Selector de enlace' + '</td> <td>' + 'Permite seleccionar al host de detino que se encuentre conectado el Host de origen seleccionado.' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Boton Generar' + '</td> <td>' + 'Permite cargar los datos regitrados en el analisis de la red.' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Host Local' + '</td> <td>' + 'Dirección IP de origen del enlace de comunicación.' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Puerto local' + '</td> <td>' + ' El puerto inicial del rango que se desea comunicar.' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Host Remoto' + '</td> <td>' + 'Dirección IP de destino del enlace de comunicación.' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Puerto Remoto' + '</td> <td>' + 'El puerto final del rango que se desea comunicar.' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Omitido' + '</td> <td>' + '.' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Protocolo' + '</td> <td>' + 'Protocolos utilizados para enviar bits de datos, conocidos como paquetes, a través de Internet.' +
            '<li>TCP-  Transfer Control Protocol:  Protocolo para el intercambio de datos de manera segura, requeriendo la autorización de conexión entre el emisor y el receptor, o el cliente y el servidor, antes de producirse la transferencia; una vez ambas partes hayan autorizado la transmisión, podrá iniciarse el envío y recepción de datos.</li>' +
            '<li>UDP - User Datagram Protocol: Protocolo del nivel de transporte basado en el intercambio de datagramas, que permite el envío de datagramas a través de la red sin que se haya establecido previamente una conexión</li>' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Bytes Especificados Tx' + '</td> <td>' + ' Ancho de banda de destino en n bits / seg (predeterminado 1 Mbit / seg para UDP, ilimitado para TCP).' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Duración' + '</td> <td>' + 'Tiempo en segundos para transmitir.' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Ventana Deslizante' + '</td> <td>' + 'Tamaños de búfer de socket en el valor especificado. Para TCP, esto establece el tamaño de la ventana de TCP. (esto se envía al servidor y también se usa en ese lado.)' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'Bloques' + '</td> <td>' + 'Número de bloques (paquetes) para transmitir. ' + '</td></tr>');
        tableAreaInfo.append('<tr><td>' + 'TCP MSS' + '</td> <td>' + 'Tamaño máximo de segmento de TCP (MSS). El MSS suele ser el MTU - 40 bytes para el encabezado TCP / IP. Para Ethernet, el MSS es de 1460 bytes (MTU de 1500 bytes).' + '</td></tr>');
    }
